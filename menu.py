...
def add(name, img_url, description, price):#adds items to table
    ...
    cursor = connection.cursor()
    cursor.execute("""\
        INSERT INTO Menu (name, img_url, description, price) VALUES 
        ('{0}','{1}','{2}',{3})""".format(name, img_url, description, price))
    connection.commit()
    connection.close()
...
def deleteById(id): #deletes items in the table by id
    connection = sqlite3.connect('main.db')
    cursor = connection.cursor()
    cursor.execute("DELETE FROM Menu WHERE id={0}".format(id))
    connection.commit()
    connection.close()

def deleteAll():
    # TODO:
    # delete all values from table Menu
    pass
